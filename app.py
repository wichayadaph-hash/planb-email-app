...
# 📌 คลังสื่อ Sales Note หลักของคุณพลอย (ใส่ File ID ของจริงได้ตรงนี้เลยค่ะ)
DRIVE_DOCX_LINKS = {
    "Central Park (TH)": "1UrsGlV-f3OKLugCLoJH6AzhIp0O01o9t",
    "Central Park (ENG)": "1UrsGlV-f3OKLugCLoJH6AzhIp0O01o9t",
    "The 20 (TH)": "1UrsGlV-f3OKLugCLoJH6AzhIp0O01o9t" # <--- แปะ File ID จริงของ The 20
}
...
with st.sidebar:
    ...
    if input_method == "แปะลิงก์ Google Sheets":
        sheet_url = st.text_input(
            ...
            help="วางลิงก์ Google Sheets (ตั้งค่าแชร์เป็น Anyone with the link)"
        )
        if sheet_url:
            try:
...
if "01" in step:
    ...
    with col1:
        ...
        selected_media_name = st.selectbox("📌 เลือกสื่อ Sales Note:", list(DRIVE_DOCX_LINKS.keys()))
    ...
    if st.button("🚀 ดึงไฟล์ Word ของสื่อที่เลือก และประมวลผล", type="primary"):
        file_target = DRIVE_DOCX_LINKS[selected_media_name]
        with st.spinner(f"กำลังดึงข้อมูลสื่อ '{selected_media_name}' จากคลังกลาง..."):
            try:
                raw_html, subject, image_store = convert_docx_to_perfect_html(file_target)
